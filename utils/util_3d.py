...
def save_mesh_as_gif(mesh_renderer, mesh, nrow=3, fps=450, out_name='1.gif'):
    ...
    imageio.mimsave(out_name, rot_comb_img, duration=duration, loop=0, disposal=2)
...
def sdf_to_mesh(sdf, level=0.02, color=None, render_all=False, demo=False):
    # device='cuda'
    device=sdf.device
    ...
    if not render_all:
        if demo:
            cprint('Warning! At most 6 meshes will be shown', 'cyan')
        elif bs > 16:
            cprint('Warning! At most 16 meshes will be shown', 'cyan')
        
        if demo:
            nimg_to_render = min(bs, 6)
        else:
...
def sdf_to_mesh_4_channel(sdf, level=0.02, color=None, render_all=False):
    # device='cuda'
    device=sdf.device
    ...
    for i in range(nimg_to_render):
        sdf_i_mof = sdf[i, 0].detach().cpu().numpy()
        sdf_i_node1 = sdf[i, 1].detach().cpu().numpy()
        sdf_i_node2 = sdf[i, 2].detach().cpu().numpy()
        sdf_i_edge = sdf[i, 3].detach().cpu().numpy()
        # verts_i, faces_i = mcubes.marching_cubes(sdf_i, 0.02)
        verts_i_mof, faces_i_mof = mcubes.marching_cubes(sdf_i_mof, level)
        verts_i_mof = verts_i_mof / n_cell - .5 

        verts_i_node1, faces_i_node1 = mcubes.marching_cubes(sdf_i_node1, level)
        verts_i_node1 = verts_i_node1 / n_cell - .5 

        verts_i_node2, faces_i_node2 = mcubes.marching_cubes(sdf_i_node2, level)
        verts_i_node2 = verts_i_node2 / n_cell - .5 

        verts_i_edge, faces_i_edge = mcubes.marching_cubes(sdf_i_edge, level)
        verts_i_edge = verts_i_edge / n_cell - .5 

        verts_i_mof = torch.from_numpy(verts_i_mof).float().to(device)
        faces_i_mof = torch.from_numpy(faces_i_mof.astype(np.int64)).to(device)
        text_i_mof = torch.ones_like(verts_i_mof).to(device)

        verts_i_node1 = torch.from_numpy(verts_i_node1).float().to(device)
        faces_i_node1 = torch.from_numpy(faces_i_node1.astype(np.int64)).to(device)
        text_i_node1 = torch.ones_like(verts_i_node1).to(device)

        verts_i_node2 = torch.from_numpy(verts_i_node2).float().to(device)
        faces_i_node2 = torch.from_numpy(faces_i_node2.astype(np.int64)).to(device)
        text_i_node2 = torch.ones_like(verts_i_node2).to(device)

        verts_i_edge = torch.from_numpy(verts_i_edge).float().to(device)
        faces_i_edge = torch.from_numpy(faces_i_edge.astype(np.int64)).to(device)
        text_i_edge = torch.ones_like(verts_i_edge).to(device)


        if color is not None:
            for i in range(3):
                text_i_mof[:, i] = color[i]
                text_i_node1[:, i] = color[i]
                text_i_node2[:, i] = color[i]
                text_i_edge[:, i] = color[i]

        verts_mof.append(verts_i_mof)
        faces_mof.append(faces_i_mof)
        verts_rgb_mof.append(text_i_mof)

        verts_node1.append(verts_i_node1)
        faces_node1.append(faces_i_node1)
        verts_rgb_node1.append(text_i_node1)

        verts_node2.append(verts_i_node2)
        faces_node2.append(faces_i_node2)
        verts_rgb_node2.append(text_i_node2)

        verts_edge.append(verts_i_edge)
        faces_edge.append(faces_i_edge)
        verts_rgb_edge.append(text_i_edge)

    try:
        p3d_mesh_mof = pytorch3d.structures.Meshes(verts_mof, faces_mof, textures=pytorch3d.renderer.Textures(verts_rgb=verts_rgb_mof))
    except:
        p3d_mesh_mof = None

    try:
        p3d_mesh_node1 = pytorch3d.structures.Meshes(verts_node1, faces_node1, textures=pytorch3d.renderer.Textures(verts_rgb=verts_rgb_node1))
    except:
        p3d_mesh_node1 = None

    try:
        p3d_mesh_node2 = pytorch3d.structures.Meshes(verts_node2, faces_node2, textures=pytorch3d.renderer.Textures(verts_rgb=verts_rgb_node2))
    except:
        p3d_mesh_node2 = None

    try:
        p3d_mesh_edge = pytorch3d.structures.Meshes(verts_edge, faces_edge, textures=pytorch3d.renderer.Textures(verts_rgb=verts_rgb_edege))
    except:
        p3d_mesh_edge = None
    ...
    try:
        ...
        p3d_mesh_node1 = pytorch3d.structures.Meshes(verts_node1, faces_node1, textures=pytorch3d.renderer.Textures(verts_rgb=verts_rgb_node1))
        p3d_mesh_node2 = pytorch3d.structures.Meshes(verts_node2, faces_node2, textures=pytorch3d.renderer.Textures(verts_rgb=verts_rgb_node2))
        p3d_mesh_edge = pytorch3d.structures.Meshes(verts_edge, faces_edge, textures=pytorch3d.renderer.Textures(verts_rgb=verts_rgb_edge))   
    ...
    return p3d_mesh_mof, p3d_mesh_node1, p3d_mesh_node2, p3d_mesh_edge
...
def render_isosurface(mesh_renderer, sdf, level=0.5, color=None, render_imsize=256, render_all=False):
    """ 
        shape of sdf:
        - bs, 1, nC, nC, nC 
...
def mesh_snapshot(mesh_renderer, mesh):
        
    img = render_mesh(mesh_renderer, mesh, norm=False) # b c h w
    img = img.permute(0, 2, 3, 1) # b h w c
    img = img.detach().cpu().numpy()
    img = (img * 255).astype(np.uint8)
